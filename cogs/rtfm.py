...
class API:
    ...
    async def feeds_create(self, ctx, *, name: str):
        ...
        exists = await self.bot.pool.fetchrow(query, ctx.channel.id, name)
        ...
        try:
            await self.bot.pool.execute(query, role.id, ctx.channel.id, name)
    ...
    async def feeds_delete(self, ctx, *, feed: str):
        ...
        records = await self.bot.pool.fetch(query, ctx.channel.id, feed)
        self.get_feeds.invalidate(self, ctx.channel.id)
