...
BLOB_GUILD_ID = 329993146651901952
...
def usage_per_day(dt, usages):
    tracking_started = datetime.datetime(2018, 8, 20)
...
class Emoji:
    ...
    async def get_all_blob_stats(self, ctx):
        ...
        total_usage = await self.bot.pool.fetchrow(query)
        ...
        blob_usage = await self.bot.pool.fetch(query, list(blob_ids.keys()))
    ...
    async def get_stats_for(self, ctx, emoji):
        ...
        usage = await self.bot.pool.fetchrow(query, emoji.id)
        usage = usage[0]
    ...
    async def get_guild_stats(self, ctx):
        ...
        top = await self.bot.pool.fetch(query, ctx.guild.id)
        ...
        e.add_field(name='Top 10 used emojis', value=value)
    ...
    async def get_emoji_stats(self, ctx, emoji_id):
        ...
        records = await self.bot.pool.fetch(query, emoji_id)
        ...
        e.add_field(name="Emoji name", value=emoji.name, inline=True)
