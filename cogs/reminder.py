...
class Reminder:
    ...
    @commands.group(aliases=['timer'], usage='<when>', invoke_without_command=True)
    async def reminder(self, ctx, *, when: time.UserFriendlyTime(commands.clean_content, default='\u2026')):
        ...
        await ctx.send(f"Alright I'll remind you, {ctx.author.mention}, in {delta} about: {when.arg}")
    ...
    async def reminder_list(self, ctx):
        ...
        if len(records) == 10:
            ...
            e.set_footer(text=f'You have {len(records)} reminder{"s" if len(records) > 1 else ""} active.')
